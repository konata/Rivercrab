...
3. 服务端实现读 `frameworks/<subsystem>/services/`,grep `CheckSystemUidCallingPermission` / `IsSystemAppByFullTokenID` /
   `VerifyAccessToken` / `IPCSkeleton::GetCallingTokenID` 等模式找校验入口
