...
const hooks = [
    {
        name: 'napi_module_register', symbol: 'napi_module_register', enter(args) {
        say(`napi_module_register(${args[0]})`);
    }
    },
    {
        ...
        enter(args) {
            this.out = args[4];
            say(`napi_new_instance name=${ctor(args[0], args[1])} argc=${args[2].toUInt32()} argv=${args[3]}`);
            if (traces-- > 0) {
                say(`napi_new_instance pc=${this.context.pc} lr=${this.context.lr} fp=${this.context.fp ||
                this.context.x29}`);
            }
        },
        leave(retval) {
...
const size =
    new NativeFunction(sym('napi_get_value_string_utf8'), 'int', ['pointer', 'pointer', 'pointer', 'ulong', 'pointer']);

const text = (env, value) => {
    const length = Memory.alloc(Process.pointerSize);
    if (size(env, value, NULL, 0, length) !== 0) {
        return '<error>';
    }
    const bytes = length.readU64().toNumber() + 1;
    const data = Memory.alloc(bytes);
    if (size(env, value, data, bytes, length) !== 0) {
        return '<error>';
    }
...
const ctor = (env, value) => {
    ...
    if (named(env, value, Memory.allocUtf8String('name'), out) !== 0) {
        return '<error>';
    }
...
const scan = reason => {
    ...
    for (const item of hooks) {
        const target =
            item.module ? Process.getModuleByName(item.module).findExportByName(item.symbol) : sym(item.symbol);
        if (hook(item.name, target, {
            onEnter(args) {
                item.enter ? item.enter(args) : say(`${item.name}()`);
            },
            onLeave(retval) {
                if (item.leave) {
                    item.leave(retval);
...
const boot = (stage = 'manual', params = {}) => {
    ...
    seen.add('boot');
    say(`boot ${stage} ${JSON.stringify(params)}`);
    ['dlopen', 'dlopen_ns', 'android_dlopen_ext'].forEach(load);

    hook('resolver:dlsym', sym('dlsym'), {
        onEnter(args) {
            this.symbol = read(args[1]) ?? '<null>';
        },
        onLeave(retval) {
            if (!watch.has(this.symbol)) {
                return;
            }
            say(`dlsym("${this.symbol}") => ${retval}`);
            if (!retval.isNull()) {
                scan(`dlsym:${this.symbol}`);
            }
        },
    });
