...
export default class Crab extends AbilityStage {
  onCreate(): void {
    const state = crab.status();
    bark(`AbilityStage:status=${JSON.stringify(state)}`);
    ...
    bark(`AbilityStage:bootstrap=${JSON.stringify(crab.bootstrap('libgadget.so', config))}`);
  }
}
